        ...
        {activeTab === 'site' && (
          <div className="space-y-8 animate-in fade-in duration-300">
            ...
            >
              ...
              {/* CORPO DO SITE MODELO */}
              <div className="bg-slate-950 text-slate-100">
                {/* Navbar Interna do Site do Cliente */}
                <header className="px-6 py-4 border-b border-slate-800/80 flex items-center justify-between sticky top-0 z-20 bg-slate-950/90 backdrop-blur-md">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-violet-600 to-sky-500 flex items-center justify-center font-black text-white text-xs shadow-md">
                      {lead.name.charAt(0)}
                    </div>
                    <div>
                      ...
                      <div className="text-[10px] text-emerald-400 font-bold flex items-center gap-1 mt-0.5">
                        ...
                        <span>Aberto Agora • Atendimento Rápido</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="px-3.5 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 text-xs font-black flex items-center gap-1.5 shadow-md shadow-emerald-900/30 transition-all"
                  >
                    <MessageSquare className="w-3.5 h-3.5" />
                    <span>WhatsApp</span>
                  </a>
                ...
                <div className="relative min-h-[360px] sm:min-h-[460px] flex items-center justify-center overflow-hidden">
                  ...
                  {/* Conteúdo do Hero */}
                  <div className="relative z-10 p-6 sm:p-12 text-center max-w-3xl space-y-4">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-extrabold bg-white/10 backdrop-blur-md text-white border border-white/20">
                      <Sparkles className="w-3 h-3 text-amber-300" />
                      {showcase.heroTag} • {lead.city}
                    </span>

                    <h2 className="text-2xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight leading-tight drop-shadow-md">
                      {showcase.heroHeadline}
                    </h2>

                    <p className="text-xs sm:text-sm text-slate-200 font-medium leading-relaxed max-w-2xl mx-auto drop-shadow">
                      {showcase.heroSubheadline}
                    </p>
                    ...
                    <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="px-6 py-3.5 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 text-xs sm:text-sm font-black flex items-center gap-2 shadow-xl shadow-emerald-950 transition-all hover:scale-105 active:scale-95"
                      >
                ...
                <div className="p-6 sm:p-10 border-y border-slate-900 bg-slate-900/40">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {showcase.benefits.map((b, i) => (
                      <div
                        key={i}
                      ...
                      >
                        ...
                        <p className="text-xs font-semibold text-slate-300 leading-snug">{b}</p>
                      </div>
                    ))}
                  </div>
                </div>

                {/* CATÁLOGO / PORTFÓLIO DE PRODUTOS & SERVIÇOS COM FOTOS ELABORADAS */}
                <div className="p-6 sm:p-12 space-y-8">
                  <div className="text-center max-w-2xl mx-auto space-y-2">
                    ...
                    <h3 className="text-2xl sm:text-3xl font-black text-white">
                      {showcase.catalogTitle}
                    </h3>
                    <p className="text-xs sm:text-sm text-slate-400">
                      {showcase.catalogSubtitle}
                    </p>
                  </div>

                  {/* Grid com Fotos Elaboradas */}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {showcase.items.map((item) => (
                      <div
                        key={item.id}
                        className="group bg-slate-900/80 border border-slate-800 rounded-3xl overflow-hidden hover:border-violet-500/50 transition-all duration-300 shadow-xl flex flex-col"
                      >
                        ...
                        <div className="relative h-52 sm:h-60 overflow-hidden">
                          ...
                          {/* Tag de Categoria e Badge */}
                          <div className="absolute top-3 left-3 flex items-center gap-2">
                            <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-black/60 text-white backdrop-blur-md border border-white/10">
                              {item.categoryTag}
                            </span>
                            {item.badge && (
                              <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-amber-400 text-slate-950 shadow-md">
                                {item.badge}
                              </span>
                            )}
                          </div>
                          ...
                          {/* Preço em Destaque */}
                          {item.priceTag && (
                            <div className="absolute bottom-3 right-3 px-3 py-1.5 rounded-xl bg-slate-950/90 text-emerald-400 font-black text-xs border border-emerald-500/30 backdrop-blur-md shadow-lg">
                              {item.priceTag}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                          ...
                          <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between">
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="px-3 py-2 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-xs font-bold flex items-center gap-1.5 transition-all shadow-md"
                            >
                ...
                {/* PROVA SOCIAL COM COMENTÁRIOS DO GOOGLE */}
                <div className="p-6 sm:p-12 border-t border-slate-900 bg-slate-900/30 space-y-6">
                  <div className="text-center max-w-xl mx-auto">
                    <h4 className="text-lg font-black text-white">
                      O que os clientes já dizem sobre a {lead.name}
                    </h4>
                    <p className="text-xs text-slate-400 mt-1">
                      Comentários autênticos e nota {lead.rating.toFixed(1)}★ no Google Maps
                    </p>
                  </div>
                    ...
                    {lead.reviews.map((rev, idx) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-extrabold text-white">{rev.author}</span>
                        ...
                        <span className="text-[10px] text-slate-500 block">{rev.relativeTime}</span>
                      </div>
                    ))}
                  </div>
                </div>

                {/* RODAPÉ DO SITE MODELO */}
                <footer className="p-6 sm:p-8 border-t border-slate-800 text-center text-xs text-slate-500 space-y-2">
                  <p className="font-bold text-slate-400">
                    © {lead.name} • {lead.address} • {lead.city} - {lead.state}
                  </p>
                  <p className="text-[11px] text-slate-600">
                    Protótipo interativo desenvolvido com Next.js, CDN de alta velocidade e integração direta com WhatsApp.
                  </p>
                </footer>
              </div>
            </div>
